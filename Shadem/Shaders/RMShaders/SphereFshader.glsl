...
{
    float sphere = distance_from_sphere(p, SpherePos, 1.0);
...
void main()
{
    ...
    vec2 uv = vec2(gl_FragCoord.x/res.x, gl_FragCoord.y/res.y) * 2 - 1;
    //vec2 uv = vec2(gl_FragCoord.x, gl_FragCoord.y);
    vec3 ro = CameraPos;
    vec3 rd = vec3( normalize( vec4(uv, 45.0f/Zoom, 0.0) ));
    //rd = vec3(uv, 1.0);
