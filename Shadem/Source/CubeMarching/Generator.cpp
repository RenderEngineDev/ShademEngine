...
#include <iostream>
#include <vector>
#include "CubeMarching/Generator.h"
...
std::vector<std::vector<std::vector<float>>> Generator::generate_sphere(glm::vec3 position, float radius) {
	//return generate_sphere(position.x, position.y, position.z, radius);
	return generate_sphere(GRID_MAX/2, GRID_MAX/2, GRID_MAX/2, radius);
}
